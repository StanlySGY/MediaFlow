...
./build.sh --save
#    产物：mediaflow-1.7.2-arm64.tar.gz
...
# 3. 现场服务器（离线）：加载镜像
docker load -i mediaflow-1.7.2-arm64.tar.gz
...
`docker-compose.prod.yml` 刻意不含 `build:`——镜像没加载成功时直接报 `image not found`，而不是悄悄尝试离线构建再失败；它固定引用 `mediaflow:1.7.2`，多次交付时现场版本可追溯。`./outputs`、`./temp` 已挂载为数据卷，更新镜像（重新 `docker load` + recreate）不丢历史结果。
