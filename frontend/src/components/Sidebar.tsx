    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-border">
        ...
        <div>
          ...
          <div className="text-[11px] text-muted">音视频处理控制台 v1.7.3</div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-4 px-3 flex flex-col gap-1">
        ...
        {workspaceItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => onViewChange(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left cursor-pointer select-none transition-colors duration-150 border ${isActive ? 'bg-accent-soft text-accent border-accent/20' : 'text-fg-dim hover:bg-surface-3 border-transparent'}`}>
        ...
        <div className="mt-5 px-3 pb-1.5 text-[11px] font-semibold text-muted">管理</div>
        {adminItems.map((item) => {
          const Icon = item.icon;
          const isActive = currentView === item.id;
          return (
        ...
        <div className="mt-5 px-3 pb-1.5 text-[11px] font-semibold text-muted">参考文档</div>

        <a href="/docs" target="_blank" rel="noreferrer"
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-fg-dim hover:bg-surface-3 transition-colors">
      ...
      <div className="p-4 border-t border-border bg-surface-2 flex flex-col gap-2.5 text-[11px]">
        <div className="flex justify-between items-center">
          ...
          <span className="text-fg-dim truncate max-w-[120px]" title={config?.provider || '—'}>
            {config?.provider || '—'}
          </span>
        </div>
        <div className="flex justify-between items-center">
