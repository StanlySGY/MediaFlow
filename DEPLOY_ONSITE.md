...
> 适用版本：`mediaflow:1.7.2`（**arm64**）。面向离线内网环境，全程无需联网。
...
| 文件 | 说明 |
|---|---|
| `mediaflow-1.7.2-arm64.tar.gz` | Docker 镜像离线包（arm64） |
...
| 启动报 `image not found` | 镜像没加载或版本号不符 | `docker images` 确认有 `mediaflow:1.7.2`，与 compose 的 `image:` 一致 |
