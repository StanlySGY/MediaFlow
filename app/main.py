...
如果实时 SSE 返回 `...mock partial...` 或 `Mock final transcription.`，说明服务当前仍在使用
`REALTIME_ASR_PROVIDER=realtime_mock`。这是演示/联调用的假数据 provider，不会调用真实 ASR。

要让两个标准接口都走真实 Qwen ASR，至少需要这样配置：
...
也可以在 Web 页面「服务配置」里设置同样的值：

- 接口类型：`openai_chat_audio`
- 接口地址：`https://dashscope.aliyuncs.com/compatible-mode/v1`
- API Key：你的 DashScope API Key
- 模型名称：`qwen3-asr-flash`
- 实时接口类型：`realtime_offline`

配置后先点「测试连接」或调用 `POST /asr/ping`。`ok=true` 之后，`/asr/file/{task_id}/events`
会直接使用真实 Qwen ASR；`/asr/realtime/{session_id}/events` 在收到 `is_final=true` 或
`POST /asr/realtime/{session_id}/end` 后，会把已收到的 base64 音频合成完整音频，再调用同一个
...
`source_event=online` 表示中间结果，`source_event=final` 表示最终文本。后续如果换成
原生 realtime ASR，调用方仍然使用同一套统一 SSE 格式。
...
文件接口 SSE 输出格式与实时接口一致，也是 `event: message`：
